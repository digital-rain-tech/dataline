...
const ENGLISH_NAMES_MALE: &[&str] = &[
    "Peter", "David", "John", "Michael", "William", "James", "Robert", "Kevin", "Eric", "Jason",
    "Andy", "Tony", "Raymond", "Patrick", "Kenneth", "Henry", "Thomas", "Chris", "Danny", "Samuel",
];
...
fn generate_csv(path: &str, num_persons: usize) {
    ...
    for pid in 0..num_persons {
        ...
        let (given_trad, given_simp) = if xorshift(&mut rng) % 100 < 15 {
            ...
            let pool = if is_female {
                GIVEN_BIGRAMS_FEMALE_GENZ
            } else {
                GIVEN_BIGRAMS_MALE_GENZ
            };
            let (t, s) = pool[(xorshift(&mut rng) % pool.len() as u64) as usize];
            (t.to_string(), s.to_string())
        } else {
        ...
        // English name (64%)
        let english_names = if is_female {
        ...
        let phone_prefix = if xorshift(&mut rng) % 2 == 0 {
            "9"
        } else {
            "6"
        };
        ...
        // Variant 1: CRM — Chinese with honorific (always)
        {
            ...
            let ph = if xorshift(&mut rng) % 100 < 80 {
                &phone
            } else {
                ""
            };
        ...
        // Variant 2: Billing — HKID romanization (always)
        {
            ...
            let dist = if xorshift(&mut rng) % 100 < 70 {
                district
            } else {
                ""
            };
        ...
        if xorshift(&mut rng) % 2 == 0 {
            ...
            let ph = if xorshift(&mut rng) % 100 < 40 {
                &phone
            } else {
                ""
            };
        ...
        if english_name.is_some() && xorshift(&mut rng) % 100 < 30 {
            ...
            let ph = if xorshift(&mut rng) % 100 < 70 {
                &phone
            } else {
                ""
            };
        ...
        if (pid + 1) % 100_000 == 0 {
            ...
            eprintln!(
                "  generated {}/{} persons ({} records) in {:.1}s",
                pid + 1,
                num_persons,
                total_records,
                elapsed.as_secs_f64()
...
fn run_pipeline(records: &[Record]) {
    ...
    for (i, record) in records.iter().enumerate() {
        ...
        for skey in &surname_keys {
            ...
            if !phone_last4.is_empty() {
                index
                    .entry(format!("{skey}|ph:{phone_last4}"))
                    .or_default()
                    .push(i);
            }
    ...
    let total_pairs: u64 = block_sizes
        .iter()
    ...
    let processable_blocks: Vec<(&String, &Vec<usize>)> = index
        .iter()
    ...
    eprintln!(
        "  Processable blocks: {} (of {})",
        num_processable, num_blocks
    );
            ...
            for (i, &left_idx) in indices.iter().enumerate() {
                for &right_idx in &indices[i + 1..] {
                    ...
                    let (score, classification) = match result.decision.classification {
                        ...
                        MatchConfidence::Review | MatchConfidence::NonMatch => {
                            (0.0, MatchClass::NonMatch)
                        }
                    };
    ...
    let matches: usize = all_match_results
        .iter()
        .filter(|r| r.classification == MatchClass::Match)
        .count();
    let possible: usize = all_match_results
        .iter()
    ...
    eprintln!(
        "  Matches:          {} (Definite: {}, High: {})",
        matches, definite, high
    );
    eprintln!("  Possible:         {} (Medium: {})", possible, medium);
    eprintln!(
        "  Throughput:       {:.0} pairs/sec",
        pairs_processed as f64 / matching_time.as_secs_f64()
    );
    ...
    eprintln!(
        "  Blocks skipped:   {} (>{MAX_BLOCK_SIZE} records)",
        skipped
    );
...
fn main() {
    ...
    if args.len() < 2 {
        ...
        eprintln!();
        eprintln!("Commands:");
        eprintln!("  pipeline <input.csv> [job_dir] [--rules <path>]   run full pipeline, write SQLite + CSV");
        ...
        eprintln!();
        eprintln!("Options:");
        eprintln!("  --rules <path>    Load rules from JSON (default: data/rules.json)");
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  cargo run --release --bin dataline-demo -- pipeline data/sample_1m.csv");
        eprintln!("  cargo run --release --bin dataline-demo -- pipeline data/sample_1m.csv data/job_1m --rules data/rules.json");
    ...
    match args[1].as_str() {
        ...
        "pipeline" => {
            ...
            let phase2a_matches =
                dataline::pipeline::phase2a_validate_clusters(&prepared, &clusters);
            ...
            eprintln!(
                "\n=== Stage 1: Build Blocks (remaining {} records) ===",
                remaining_count
            );

            let remainder_job_dir = format!("{}/remainder", job_dir);
            let remainder_job =
                dataline::pipeline::JobDir::new(std::path::Path::new(&remainder_job_dir));
            ...
            let t = Instant::now();
            let mut db =
                dataline::pipeline::results_db::ResultsDb::open(std::path::Path::new(&db_path));
            ...
            for cluster in &clusters {
                ...
                let record_ids: Vec<String> = cluster
                    .member_indices
                    .iter()
            ...
            let total_matches = phase1_matches.len()
                + phase2a_matches.len()
                + phase2_matches.len()
                + stage2_matches_count;
            ...
            eprintln!("  Records:       {}", records.len());
            eprintln!(
                "  Phase 1:       {} clusters, {} matches (deterministic)",
                clusters.len(),
                phase1_matches.len()
            );
            eprintln!(
                "  Phase 2:       {} matches (targeted comparison)",
                phase2_matches.len()
            );
            eprintln!(
                "  Stage 2:       {} pairs, {} matches (pairwise remainder)",
                stage2_pairs, stage2_matches_count
            );
